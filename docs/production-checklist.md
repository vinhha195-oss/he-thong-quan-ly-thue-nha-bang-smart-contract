...
|---|---|---|
| Unit test cho toàn bộ luồng nghiệp vụ chính | ✅ Đã làm | 32 test (`test/RentalManager.test.ts`): đăng tin, huỷ tin, đặt cọc, trả tiền (kèm phạt trễ), bàn giao, đề xuất/đồng ý/khiếu nại/trọng tài bỏ phiếu (multisig), CID ảnh IPFS, các trường hợp bị chặn, token đại diện hợp đồng. |
