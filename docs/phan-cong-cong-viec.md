...
| Test | Viết 32 test case (nghiệp vụ + token + IPFS CID + phạt trễ hạn + trọng tài/multisig + huỷ tin đăng + bảo mật: chặn `safeTransferFrom`, chặn reentrancy qua contract độc hại) | `test/RentalManager.test.ts`, `contracts/mocks/MaliciousReceiver.sol` |
